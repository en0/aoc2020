...
class Space:
    def __init__(self, initial_state):
        ...
        xi, yi, zi = inf, inf, inf
        xa, ya, za = -inf, -inf, -inf
        for x, y, z in self._space.keys():
            xi, yi, zi = min(xi, x), min(yi, y), min(zi, z)
            xa, ya, za = max(xa, x), max(ya, y), max(za, z)
        self._min, self._max = (xi, yi, zi), (xa, ya, za)

    def __iter__(self):
        x_min, y_min, z_min = self._min
        x_max, y_max, z_max = self._max
    ...
    def _active_around(self, point):
        ...
        return sum((
            ...
            for p in product([x+1, x-1, x], [y+1, y-1, y], [z+1, z-1, z])
            if p != point))
